...
def test_root_hello_world(authorized_client):
    response = authorized_client.get("/")
    assert response.status_code == 200
    assert response.json() == {"hello": "world"}


def test_get_movies_returns_seeded_entries(authorized_client, movie_factory):
...
def test_create_movie_persists_and_returns_new_movie(authorized_client, db_session):
    payload = {"title": "New Movie", "genres": "Comedy"}

    response = authorized_client.post("/movies", json=payload)
...
def test_get_movie_returns_specific_movie(authorized_client, movie_factory):
    movie = movie_factory(title="Original", genres="Drama|Action")

    response = authorized_client.get(f"/movies/{movie.id}")
    assert response.status_code == 200
    body = response.json()
...
def test_get_movie_missing_returns_404(authorized_client):
    response = authorized_client.get("/movies/9999")
    assert response.status_code == 404
    assert response.json()["detail"] == "Movie not found"


def test_update_movie_modifies_existing_record(authorized_client, movie_factory, db_session):
    movie = movie_factory(title="Old Title", genres="Action")
    payload = {"title": "Updated Title"}

    response = authorized_client.put(f"/movies/{movie.id}", json=payload)
    assert response.status_code == 200
    body = response.json()
...
def test_delete_movie_removes_record(authorized_client, movie_factory, db_session):
    movie = movie_factory()

    response = authorized_client.delete(f"/movies/{movie.id}")
    assert response.status_code == 204
    assert db_session.query(Movie).count() == 0


def test_get_links_returns_seeded_entries(authorized_client, link_factory, movie_factory):
    movie_a = movie_factory()
    movie_b = movie_factory()
...
def test_create_link_persists_new_entry(authorized_client, movie_factory, db_session):
    movie = movie_factory()
    payload = {"movie_id": movie.id, "imdb_id": "tt7654321", "tmdb_id": "98765"}

    response = authorized_client.post("/links", json=payload)
    assert response.status_code == 201
    body = response.json()
...
def test_get_link_returns_single_entry(authorized_client, link_factory):
    link = link_factory()

    response = authorized_client.get(f"/links/{link.movie_id}")
    assert response.status_code == 200
    body = response.json()
...
def test_update_link_overwrites_fields(authorized_client, link_factory, db_session):
    link = link_factory(imdb_id="tt1111111", tmdb_id="1111")
    payload = {"tmdb_id": "2222"}

    response = authorized_client.put(f"/links/{link.movie_id}", json=payload)
    assert response.status_code == 200
    body = response.json()
...
def test_delete_link_removes_entry(authorized_client, link_factory, db_session):
    link = link_factory()

    response = authorized_client.delete(f"/links/{link.movie_id}")
    assert response.status_code == 204
    assert db_session.query(Link).count() == 0


def test_get_ratings_returns_seeded_entries(authorized_client, rating_factory):
...
def test_create_rating_persists_new_entry(authorized_client, movie_factory, db_session):
    movie = movie_factory()
    payload = {"user_id": 77, "movie_id": movie.id, "rating": 4.5, "timestamp": 123456789}

    response = authorized_client.post("/ratings", json=payload)
    assert response.status_code == 201
    body = response.json()
...
def test_get_rating_returns_single_entry(authorized_client, rating_factory):
    rating = rating_factory(rating=2.5)

    response = authorized_client.get(f"/ratings/{rating.id}")
    assert response.status_code == 200
    body = response.json()
...
def test_get_rating_missing_returns_404(authorized_client):
    response = authorized_client.get("/ratings/9999")
    assert response.status_code == 404
    assert response.json()["detail"] == "Rating not found"


def test_update_rating_modifies_existing_resource(authorized_client, rating_factory, db_session):
    rating = rating_factory(rating=1.5)
    new_timestamp = rating.timestamp + 10
    payload = {"rating": 4.0, "timestamp": new_timestamp}

    response = authorized_client.put(f"/ratings/{rating.id}", json=payload)
    assert response.status_code == 200
    body = response.json()
...
def test_delete_rating_removes_existing_entry(authorized_client, rating_factory, db_session):
...
def test_get_tags_returns_seeded_entries(authorized_client, tag_factory):
...
def test_create_tag_persists_entry(authorized_client, movie_factory, db_session):
    movie = movie_factory()
    payload = {"user_id": 42, "movie_id": movie.id, "tag": "Favorite", "timestamp": 987654321}

    response = authorized_client.post("/tags", json=payload)
    assert response.status_code == 201
    body = response.json()
...
def test_get_tag_returns_single_entry(authorized_client, tag_factory):
    tag = tag_factory(tag="Top Pick")

    response = authorized_client.get(f"/tags/{tag.id}")
    assert response.status_code == 200
    data = response.json()
...
def test_update_tag_modifies_existing_entry(authorized_client, tag_factory, db_session):
    tag = tag_factory(tag="Old Value")
    payload = {"tag": "Updated Value"}

    response = authorized_client.put(f"/tags/{tag.id}", json=payload)
    assert response.status_code == 200
    data = response.json()
...
def test_delete_tag_removes_entry(authorized_client, tag_factory, db_session):
    tag = tag_factory()

    response = authorized_client.delete(f"/tags/{tag.id}")
    assert response.status_code == 204
